...
We have been provided with a dataset of Amazon products. In particular, the dataset consists of two JSON files: \textit{metadata}, which contains information related to the products, and \textit{reviews}, which contains the reviews and the ratings associated to each product.
\subsection{Metadata}
The file contains information about the products, such as their unique ID, description, price and the asin of related products. The size of the file is 9.81 GB (uncompressed, json format).
...
The dataset, due to its large size, could not be easily managed directly using Pandas. Therefore, it has been initially processed using PySpark, both on the cluster (especially for the reviews dataset) and locally. While it may seem inappropriate at first, using Spark in local makes sense for medium-sized datasets (like the metadata one), as it automatically parallelizes jobs using all cores, and spills to disk intermediate results that cannot fit in main memory.

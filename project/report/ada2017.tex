...
\todo[inline]{merge reviews with metadata}
\todo[inline]{5 categories selction}
...
\todo{augmentation with fan-in/out, connected components}
\todo{we did not used "also viewed"}
\todo{cartsimilarities}
...
\todo{stats (lot unconnected components, correlations)}
\todo{patterns (accumulators, cliques=competing products)}
\todo{Wrong categories}

\subsection{Methodology}
\todo{why fan-in? it is more constant in time! salesrank is not. arguments in favor of fan-in ... e.g. it correlates with sales rank}
\todo{best product=product with highest fan-in. Based on the direction of the edge we can predict which is the one with the highest fan-in}

\subsection{Predicting the best products}
\todo{machine learning for predicting the best product (highest fan-in) from text, price, #reviews, average rating}
\todo{results on non-competing products (outside the cliques)}
\todo{results on competing products (inside the cliques), as expected, are worse}

\subsection{Bias in brands}
\todo{herding effect with number of reviews}
